...
def create_user(lst):
    ...
    cnt = 0
    for user in lst:
        TF , False_list = is_validation(user)[0],is_validation(user)[1]
        if TF == True:
        ...
        if TF == False:
            for val in False_list:
                user[val] = None
            user_list.append(user) 
        if TF == False or is_validation(user) == 'blocked' :
            cnt += 1
        if TF == 'blocked':
            pass
...
def is_validation(user):
    False_list = ['blood_group','mail','name','website']
    if user['blood_group'] in blood_types:
        False_list.remove('blood_group')
    if user['company'] in black_list:
        return 'blocked'
    if '@' in user['mail'] :
        False_list.remove('mail')
    if 2 <= len(user['name']) <= 30:
        False_list.remove('name')
    if user['website']:
        False_list.remove('website')
    return (not bool(False_list),False_list)
